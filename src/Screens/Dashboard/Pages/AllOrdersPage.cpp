...
void AllOrdersPage::setupUI() {
    ...
    tableContainer->setStyleSheet("background-color: #1a2235; border-radius: 12px; border: 1px solid #283548;");
    ...
    allOrdersLayout->addWidget(new PaginationBar(this));
}

void AllOrdersPage::loadInitialData() {
...
void AllOrdersPage::setupConnections() {
    connect(filterBar, &FilterBar::addOrderRequested, this, &AllOrdersPage::handleAddOrder);
    
    connect(filterBar, &FilterBar::searchChanged, this, [this](const QString& text) {
        orderTable->applyFilters(text, filterBar->currentStatus());
    });

    connect(filterBar, &FilterBar::filterChanged, this, [this](const QString&, const QString& status) {
        orderTable->applyFilters(filterBar->currentSearch(), status);
    });

    connect(orderTable, &OrderTable::editRequested, this, &AllOrdersPage::handleEditOrder);
    connect(orderTable, &OrderTable::deleteRequested, this, &AllOrdersPage::handleDeleteOrder);
}

void AllOrdersPage::handleAddOrder() {
    AddOrderDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
        auto formData = dialog.getFormData();
        orderTable->addOrder({
            QString::number(nextOrderId++),
            formData.clientName, formData.phone, formData.address,
            formData.product, formData.size, formData.isPainted, formData.status
        });
    }
}

void AllOrdersPage::handleEditOrder(const QString& id) {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        orderTable->updateOrder({
            id, updatedForm.clientName, updatedForm.phone, updatedForm.address,
            updatedForm.product, updatedForm.size, updatedForm.isPainted, updatedForm.status
        });
    }
}

void AllOrdersPage::handleDeleteOrder(const QString& id) {
